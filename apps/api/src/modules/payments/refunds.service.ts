...
import { payments, refunds, type DatabaseConnection, type Payment } from '@bokku/database';
...
export class RefundsService {
  ...
  async request(payment: Payment): Promise<void> {
    if (payment.status !== 'SUCCESS') throw new Error(`Payment ${payment.reference} is not refundable`);
    ...
    if (refund && ACTIVE.has(refund.status)) return;
  ...
  async handlePaystackWebhook(rawBody: Buffer | undefined, signature: string | undefined): Promise<{ received: boolean }> {
    ...
    if (!current) {
      await this.database.db.insert(refunds).values({
        ...
        providerReference,
        providerStatus: next,
        failureReason: next === 'FAILED' ? String(event.data.reason ?? 'PAYSTACK_REFUND_FAILED') : null,
        processedAt: next === 'PROCESSED' ? new Date() : null,
      });
    } else {
      await this.database.db.update(refunds).set({
        status: next,
        providerStatus: next,
        ...(providerReference ? { providerReference } : {}),
        ...(next === 'FAILED' ? { failureReason: String(event.data.reason ?? 'PAYSTACK_REFUND_FAILED') } : {}),
